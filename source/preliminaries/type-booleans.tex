...
实际上, 可以观察到, 有从 $\Sigma$-types 和 $\bool$ 得到出二元余积的另一种方式.

\index{递归原理!布尔类型的}
为了得到函数 $f : \bool \to C$, 需要 $c_0,c_1 : C$ 并添加定义等同
\begin{align*}
    f(\bfalse) &\defeq c_0, \\
    f(\btrue) &\defeq c_1.
\end{align*}
...
\index{归纳原理!布尔类型的}
给定 $C : \bool \to \UU$, 为了得到一个依赖函数 $f : \prd{x:\bool}C(x)$, 需要 $c_0:C(\bfalse)$ 和 $c_1 : C(\btrue)$, 在这中情况下可以给出定义等式
\begin{align*}
    f(\bfalse) &\defeq c_0, \\
    f(\btrue) &\defeq c_1.
\end{align*}
...
和以前一样, 为了陈述它, 需要使用还没有被引入的等同类型, 只需要使用所有东西等于它自己这个事实, 即 $\refl{x}:x=x$.
因此, 构造一个元素, 它的类型是
\begin{equation}
...
可以使用 \bool 的归纳原理, 其中 $C(x) \defeq (x=\bfalse)+(x=\btrue)$;
...
会在 \cref{sec:htpy-inductive} 回到这个问题.
