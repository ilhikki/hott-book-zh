\index{类型!恒等|(}%
就像类型 \id[A]{a}{a'} 定性为同构, 每个 $A$ 都有单独的 ``定义'', 路径 $p,q : \id[A]{a}{a'}$ 之间的路径 \id[{\id[A]{a}{a'}}]{p}{q} 却不能轻易定性.
...
因此, 如果对于类型 $A$ 能对 $\id[A]{a}{a'}$ 充分定性, 类型 $\id[{\id[A]{a}{a'}}]{p}{q}$ 也是确定的.
例如:
\begin{itemize}
